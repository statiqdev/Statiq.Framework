...
namespace Statiq.Common
{
    ...
    /// </summary>
    // From https://github.com/aspnet/AspNetCore/blob/b617083148b17e100dfe5552f753b9a98e6935df/src/Middleware/StaticFiles/src/FileExtensionContentTypeProvider.cs
    public static class MediaTypes
    {
        ...
        /// as well as conventional unregistered media types.
        /// This dictionary can be edited prior to execution to customize media types.
        /// </summary>
            ...
            new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                /*
                    async Task Main()
                    {
                        ...
                        {
                            string result = await client.GetStringAsync("http://svn.apache.org/repos/asf/httpd/httpd/trunk/docs/conf/mime.types");
                                ...
                                while ((line = lines.ReadLine()) is object)
                                {
                                    if (!line.StartsWith("#"))
                                    {
                                        ...
                                        if (parts.Length > 1)
                                        {
                                            ...
                                            {
                                                if (!mappings.ContainsKey(parts[c]))
                                                {
                                                    mappings.Add(parts[c], parts[0]);
                                                }
                                            }
                                        }
                                    }
                        ...
                        string.Join(
                            Environment.NewLine,
                            mappings.OrderBy(x => x.Key).Select(x => $"{{ \".{x.Key}\", \"{x.Value}\" }},"))
                            .Dump();
                    }
                */
        ...
        public static bool TryGet(string path, out string mediaType, bool defaultIfNotFound = true)
        {
            ...
            if (ExtensionMappings.TryGetValue(extension, out mediaType))
            {
                return true;
            }
            if (defaultIfNotFound)
            {
                mediaType = "text/x-" + extension.Substring(1);
                return true;
            }
            return false;
        }
