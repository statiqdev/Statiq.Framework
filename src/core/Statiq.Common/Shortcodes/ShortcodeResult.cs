...
namespace Statiq.Common
{
    public class ShortcodeResult
    {
        ...
        public ShortcodeResult(Stream content, IEnumerable<KeyValuePair<string, object>> nestedMetadata = null)
            : this(content == null ? null : IExecutionContext.Current.GetContentProvider(content), nestedMetadata)
        {
        }

        public ShortcodeResult(string content, IEnumerable<KeyValuePair<string, object>> nestedMetadata = null)
            : this(content == null ? null : IExecutionContext.Current.MemoryStreamFactory.GetStream(content), nestedMetadata)
        {
        }
        ...
        public static implicit operator ShortcodeResult(Stream content) => content == null ? null : new ShortcodeResult(content);
