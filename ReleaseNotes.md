...
- Documents are not longer tracked by the execution context and calling `IExecutionContext.GetDocument()` is no longer required (though it's still prefered in order to populate documents with default settings metadata)
...
- A new `ObjectDocument` document type and cooresponding `.ToDocument()` and `.ToDocuments()` extensions provide wrapping capability for any object
