Thanks for your interest in getting involved with Statiq! This project was always envisioned as a community effort. Any contributions you're able to make are **greatly appreciated**.
...
[Fork](http://help.github.com/forking/) the Statiq repository on GitHub. Please do your work in feature branches based on `develop`. `develop` is the main development branch and is where all code is pushed before making a release.
...
Generally, you should only ever have to take a dependency on `Statiq.Common`. If you find yourself needing to depend on other Statiq libraries like `Statiq.Core`, please open an issue so the needed functionality can be moved into the common assembly.
...
- Apply the "Statiq Rules" code analysis ruleset by opening Analyze, Configure Code Analysis (apply the "Statiq Test Rules" ruleset to test projects)
...
- Tests use [NUnit](https://github.com/nunit). Instead of mocks, use stub classes from `Statiq.Testing`. Add new stub behavior there if needed to support your tests.
...
Making updates to the Statiq documentation is just as helpful as writing code (if not more helpful). The Statiq documentation exists in the [Statiq.Web repository](https://github.com/statiqdev/Statiq.Web). Fork and clone the repository as you would the main code repository, but make any changes in the `master` branch. Submit your changes as a pull request. Once it's accepted, the site will automatically rebuild and redeploy.

# License
By contributing to Statiq, you assert that:
